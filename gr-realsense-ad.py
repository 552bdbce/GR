...
try:
    ...
    while True:
        ...
        for i_rep in reversed(reverse_list):
            test40 = np.zeros((401, 640))
            for j_rep in range(i_rep * 35, (i_rep + 2) * 35):
                for k_rep in range(10, 630):
                    if np_image[j_rep][k_rep] <= 400 and np_image[j_rep][k_rep] >= 100:  # 400mmまでに茎がないかどうか
                        test40[np_image[j_rep][k_rep]][k_rep] += 1
            rep1 = np.argmax(test40)
            rep2, rep3 = divmod(rep1, 640)
            x_rep[i_rep] = rep3
            y_rep[i_rep] = (i_rep + 1) * 35
            z_rep[i_rep] = rep2
            print(i_rep)
