...
def sliding_windows(cv_bin, win_left_x):
    '''
    ...
    # 画面下半分のピクセル数をカウントする
    # histogram = np.sum(cv_bin[int(360/2):, :], axis=0)
    ...
    for window in range(nwindows):
        ...
        # 左右windowの枠を描画する
        cv2.rectangle(cv_rgb_sliding_windows,(win_xleft_low,win_y_low),(win_xleft_high,win_y_high),rectangle_color, 1)  # 左上と右下の2点指定
        #cv2.rectangle(cv_rgb_sliding_windows,(win_xright_low,win_y_low),(win_xright_high,win_y_high),rectangle_color, 1)

        window_array = np.array(cv_bin[win_y_low:win_y_high, win_xleft_low:win_xleft_high], dtype=np.int32)
        values = np.unique(window_array.ravel())
        offset = 0
        result = np.zeros_like(window_array)
        if result.size != 0:
            labeled, num_features = label(window_array == 255)
            result += labeled + offset*(labeled > 0)
            offset += num_features
            pixel_count = np.zeros(3)
            pixel_count[0] = len(np.where(result ==1)[0])
            pixel_count[1] = len(np.where(result ==2)[0])
            pixel_count[2] = len(np.where(result ==3)[0])
            pixel_max = np.argmax(pixel_count) + 1
            result_ravel = result.ravel()
            result_ravel_where = np.array(list(np.where(result_ravel == pixel_max)))
            result_ravel_where_T = result_ravel_where.transpose()
            points[win_y_high][0] = np.amax(result)
            cog_sum_x = np.zeros(3)
            count_cog = np.zeros(3)
            cog_sum_x_2 = np.zeros(3)
            count_cog_2 = np.zeros(3)
            cog_x = 0
            cog_x_2 = 0
            count_cog_1_1 = 0
            count_cog_2_2 = 0
            ...
            if np.amax(result) != 1:
                if np.count_nonzero(result == 1) >= 40:  # 大きすぎるやつをはじく
                    result = np.where(result == 1, 0, result)
                if np.count_nonzero(result == 2) >= 40:
                    result = np.where(result == 2, 0, result)
            for center_i in range(0,3):
                for center_j in range(0,32):  # 幅による
                    if result[center_i, center_j] == 1:
                        cog_sum_x[center_i] += center_j
                        count_cog[center_i] += 1
                    if result[center_i, center_j] == 2:
                        cog_sum_x_2[center_i] += center_j
                        count_cog_2[center_i] += 1
                cog_x += cog_sum_x[center_i] / count_cog[center_i]
                cog_x_2 += cog_sum_x_2[center_i] / count_cog_2[center_i]
            cog_point_x_1 = cog_x / 3  # window
            cog_point_x_2 = cog_x_2 / 3  # window
            if np.amax(result) == 1:
                points[win_y_high][1] = cog_point_x_1 + win_xleft_low
            ...
            if win_num_lefts <= 85 and pixel_count[2] == 0:
                for window_x in range(0, win_xleft_high-win_xleft_low):
                    if result[0][window_x] != 0:
                        window_data_point_left = window_x
                for window_x in range(0, win_xleft_high-win_xleft_low)[::-1]:
                    if result[0][window_x] != 0:
                        window_data_point_right = window_x

                if np.isnan(cog_point_x_1) == False and win_y_high <= 479:
                    dots_y.append(win_y_high)
                    # print(cog_point_x_1)
                    dots_x.append(int(cog_point_x_1) + win_xleft_low)
                if np.isnan(cog_point_x_2) == False and win_y_high <= 479:
                    dots_y.append(win_y_high)
                    # print(cog_point_x_2)
                    dots_x.append(int(cog_point_x_2) + win_xleft_low)
                if np.amax(result) == bf_point_num+1:  # 重心座標が1増えたとき
                    candidate_1_sum = 0
                    candidate_2_sum = 0
                    if np.isnan(cog_point_x_1) == False and np.isnan(cog_point_x_2) == False:
                        branch_y.append(win_y_high)

                        candidate_1 = int(cog_point_x_1) + win_xleft_low
                        candidate_2 = int(cog_point_x_2) + win_xleft_low
                        # print(int(cog_point_x_1) + win_xleft_low, int(cog_point_x_2) + win_xleft_low)  # left, right
                        dots_y_uni = np.unique(dots_y)
                        dots_y_uni_rev = dots_y_uni[::-1]
                        if dots_y_uni_rev.size != 0:
                            # print(dots_y_uni_rev)
                            for picking_i in range(1,len(dots_y_uni_rev)):
                                if points[win_y_high+(picking_i*3)][0] != 1:
                                    break
                                candidate_1_sum += abs(candidate_1-points[win_y_high+(picking_i*3)][1])
                                candidate_2_sum += abs(candidate_2-points[win_y_high+(picking_i*3)][1])
                            if candidate_1_sum > candidate_2_sum:
                                selected_point = candidate_2
                            else:
                                selected_point = candidate_1
                            #print(selected_point)
                            branch_x.append(selected_point)



            # np.savetxt('win_left_idx.csv', win_left_idx, delimiter=',')
            #np.savetxt('windowarrau.csv', result, delimiter=',')
            #print("result", result)
            #print("number_pixel", win_num_lefts)
            ...
            if win_num_lefts <= 85 and pixel_count[2] == 0:
                ...
                if win_left_x <= start_point - 10:
                    win_left_x += ( start_point - 10 - win_left_x )
                    last_win_left_x = win_left_x
            ...
            # win_left_x += win_right_x - last_win_right_x
                bf_point_num = np.amax(result)


    # window毎の配列を結合する
    if len(lane_left_idx) != 0:
    ...
    cv_rgb_sliding_windows[left_dot_y, left_dot_x] = [low_color, low_color, high_color]
    cv_rgb_sliding_windows[branch_y, branch_x] = [high_color, high_color, high_color]
    # cv_rgb_sliding_windows[right_y, right_x] = [low_color, low_color, high_color]
    # np.savetxt('points.csv', np.hstack((branch_x,branch_y)), delimiter=',')  # toggle
    cv2.imshow('real', cv_rgb_sliding_windows)
    # cv2.imwrite('ex2.jpg', cv_rgb_sliding_windows)


    return branch_x, branch_y, left_dot_x, left_dot_y, cv_rgb_sliding_windows
